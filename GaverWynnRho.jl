...
function GWR_array(func_array, t, M)
  ...
  @inbounds  for i in 1: 2 * M #The @inbounds improves performance by preventing an internal check. Will over right other data if implemented wrong.
    ...
    println("Data type: ", Dt)#BUG Debug line
    println("Fi[1]: ", Fi[1])#BUG Debug line
    println("Type of Fi elements: ", typeof(Fi[1]))#BUG Debug line
    println("ArrayType: ", ArrayType)#BUG Debug line
    #Gm =  Array{ArrayType}(Array{Dt}(zero(Dt), size(Fi[1])...), M1+1) #BUG: Needs to actually be arrays of zeros. How big though?
    Gm = [zeros(Dt, size(Fi[1])...) for i in 1:(M1+1)]#fi[1] is a stand in for the dimensions of func_arrray.
    #Gp =  Array{ArrayType}(Array{Dt}(zero(Dt), size(Fi[1])...), M1+1)
